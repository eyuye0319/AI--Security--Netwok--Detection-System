...
const JWT_SECRET =
  process.env.JWT_SECRET || "security-monitoring-jwt-secret-key-2026";
const PORT = 3000;
...
function getRandomIP(isAttacker = false): string {
  if (isAttacker) {
    ...
    const prefix =
      maliciousSubnets[Math.floor(Math.random() * maliciousSubnets.length)];
...
function generateSimulatedPacket(forceThreat?: ThreatType): NetworkPacket {
  ...
  const threatType: ThreatType = forceThreat
    ...
    : isMalicious
      ? ([
          ...
          "DATA_EXFILTRATION",
          "ANOMALOUS_PAYLOAD",
        ][Math.floor(Math.random() * 5)] as ThreatType)
      : "NONE";
  ...
  if (threatType === "PORT_SCAN") {
    ...
    anomalyScore = Number((Math.random() * 0.25 + 0.7).toFixed(3));
  ...
  if (threatType !== "NONE") {
    ...
    if (anomalyScore >= 0.7 && alertsList.length < 50) {
...
async function startServer() {
  ...
  app.get("/api/network/traffic", (req: Request, res: Response) => {
    ...
    const avgEntropy = (
      packetBuffer.reduce((acc, p) => acc + p.payload_entropy, 0) /
      (totalPackets || 1)
    ).toFixed(2);
    ...
    packetBuffer.forEach((p) => {
      if (p.threat_type !== "NONE") {
        attackTypeCounts[p.threat_type] =
          (attackTypeCounts[p.threat_type] || 0) + 1;
      }
    });
    ...
    res.json({
      ...
      metrics: {
        ...
        clean_traffic_pct: (
          ((totalPackets - threats.length) / (totalPackets || 1)) *
          100
        ).toFixed(1),
        malicious_traffic_pct: (
          (threats.length / (totalPackets || 1)) *
          100
        ).toFixed(1),
        average_payload_entropy: avgEntropy,
        attack_distribution: Object.entries(attackTypeCounts).map(
  ...
  app.post("/api/network/simulate-attack", (req: Request, res: Response) => {
    ...
    const chosenAttack = validAttacks.includes(attack_type)
      ? attack_type
      : "DDOS";
  ...
  app.post("/api/ml/predict", (req: Request, res: Response) => {
    ...
    if (isSyn && !isAck && freq > 20) score += 0.3;
    ...
    res.json({
      ...
      model_metrics: {
        ...
        feature_importance: [
          ...
          { feature: "packet_size", importance: 0.1 },
        ],
      },
    });
  });
...
`;
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(
      `🛡️ Network Security Platform running on http://0.0.0.0:${PORT}`,
    );
  });
}
